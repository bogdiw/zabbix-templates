...
def build():
    ...
    if_proto = [
        {"uuid":U("proto:ifOperStatus"),"name":"Interface {#IFNAME}: Operational status",
         ...
         "trigger_prototypes":[{"uuid":U("triggerproto:iflinklost"),
            "expression":f"last(/{TPL}/net.if.status[{{#SNMPINDEX}}],#1)=2 and last(/{TPL}/net.if.status[{{#SNMPINDEX}}],#2)=1 and last(/{TPL}/net.if.adminstatus[{{#SNMPINDEX}}])=1",
            "name":"Interface {#IFNAME} link lost on {HOST.NAME}","priority":"AVERAGE",
            "manual_close":"YES"}]},
